...
Execution creates a private durable claim keyed by the validated XRPL transaction hash and a separate live-process lock keyed by the executor address, then persists every state transition in the transaction-specific `SENT_FILE`. A second process using the same signing key cannot proceed, race its account nonce, or overwrite executor state while the first is live—even for a different XRP payment. A crash-stale lock is never stolen automatically: confirm the recorded process is dead before removing it. FDC request bytes, the signed FDC request transaction, voting round, decoded proof, signed AssetManager transaction, attempts, and delay timestamp survive a restart.
