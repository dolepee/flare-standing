...
describe("Standing atomic subscription instruction", () => {
  it("encodes exact approve then openMandate calls", () => {
    const calls = buildStandingCalls({ fxrp, standing, planId: 4n, depositAtomic: 1_000_000n });
  ...
  it("rejects invalid amounts and instruction shapes", () => {
    ...
    expect(() => buildStandingCalls({ fxrp, standing, planId: 0n, depositAtomic: 1n })).toThrow("planId");
    const calls = buildStandingCalls({ fxrp, standing, planId: 4n, depositAtomic: 1n });
    expect(() => encodeHashInstruction({ calls: calls.slice(0, 1), sender, nonce: 0n })).toThrow("exactly two");
  });
});
