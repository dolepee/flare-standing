...
export const ATOMIC_PROOF = Object.freeze({
  ...
  openingBlock: '33,893,083',
  recurringBlock: '33,893,456',
...
export const ATOMIC_REPLAY_STEPS: readonly AtomicReplayStep[] = Object.freeze([
  ...
  },
  {
    ...
    summary:
      `Inside that same Coston2 transaction, the Smart Account approved Standing, opened mandate 1 for plan 1, and charged the first 0.1 FTestXRP cycle. At block ${ATOMIC_PROOF.openingBlock}, the merchant received 0.099, the protocol fee was 0.001, and 0.9 remained under the subscriber-controlled mandate.`,
    result: `Opening snapshot · block ${ATOMIC_PROOF.openingBlock} · 0.9 left`,
    transaction: ATOMIC_PROOF.coston2Transaction,
    href: ATOMIC_PROOF.coston2Href,
  ...
  },
  {
    ...
    index: '04',
    network: 'Flare Coston2 testnet',
    title: 'A different keeper advanced the next cycle.',
    summary:
      `The permissionless keeper address later charged mandate 1 without the subscriber key. At block ${ATOMIC_PROOF.recurringBlock}, the second 0.1 FTestXRP cycle settled with the same 0.099 merchant and 0.001 fee split, leaving 0.8 under the mandate.`,
    result: `Recurrence snapshot · block ${ATOMIC_PROOF.recurringBlock} · 0.8 left`,
